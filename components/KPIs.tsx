...
type KPIItem = {
  label: React.ReactNode; // ⬅️ allow <Tx> or strings
  value: number;
  prefix?: string;
  suffix?: string;
  decimals?: number;
...
export default function KPIs({ items }: { items: KPIItem[] }) {
  ...
  return (
    <div ref={ref} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {items.map((it, idx) => (
        <KpiCard key={idx} active={active} {...it} />
      ))}
    </div>
  );
}
...
function KpiCard({
  ...
  return (
    <div className="card-gradient rounded-xl border border-gray-200/60 dark:border-gray-800/60 p-4 bg-white/40 dark:bg-gray-900/30 backdrop-blur-sm">
      ...
      {spark.length > 0 && (
        <svg viewBox="0 0 120 36" className="mt-3 w-full" aria-hidden="true" focusable="false">
