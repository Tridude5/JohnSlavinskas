...
export default function ImprintClient({
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-12 text-zinc-900 dark:text-zinc-100">
      <h1 className="text-3xl font-semibold tracking-tight">{t("imprint.title")}</h1>
      <p className="mt-2 text-sm opacity-70">
        {t("imprint.lastUpdated", { date: formattedDate })}
      </p>

      <section className="mt-8 space-y-2">
        <h2 className="text-xl font-medium">{t("imprint.serviceProviderHeading")}</h2>
      ...
      <section className="mt-8 space-y-2">
        <h2 className="text-xl font-medium">{t("imprint.contactHeading")}</h2>
        <p>
          {t("imprint.phoneLabel")}{" "}
          <a className="underline underline-offset-2"
             href={`tel:${me.phone.replace(/[^\d+]/g, "")}`}>{me.phone}</a>
        </p>
        <p>
      ...
      <section className="mt-8 space-y-2">
        <h2 className="text-xl font-medium">{t("imprint.responsibleHeading")}</h2>
        <p>{me.name}</p>
      </section>
      ...
      <section className="space-y-4 leading-7">
        <h2 className="text-xl font-medium">{t("imprint.liabilityContentHeading")}</h2>
        <p>{t("imprint.liabilityContent")}</p>
        <h3 className="text-lg font-medium">{t("imprint.linksLiabilityHeading")}</h3>
        <p>{t("imprint.linksLiability")}</p>
      </section>

      <section className="mt-8 space-y-4 leading-7">
        <h2 className="text-xl font-medium">{t("imprint.copyrightHeading")}</h2>
