...
// Use repo secret if present, else fall back to the default GITHUB_TOKEN from Actions
const token = process.env.GH_TOKEN || process.env.GITHUB_TOKEN;

const outPath = path.join(process.cwd(), "public", "github-contrib.json");
await fs.mkdir(path.dirname(outPath), { recursive: true });

// If no token, don't fail the build — keep old data or write zeros
if (!token) {
...
// ✅ include private contributions in the weekly totals
const query = `
